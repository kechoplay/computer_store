                ...
                <div class="x_panel">
                    ...
                    <div class="x_content">
                        ...
                        <form method="post" data-parsley-validate class="form-horizontal form-label-left"
                              action="{{ route('updateSale', ['id' => $khuyenmai->id]) }}">
                            {{ csrf_field() }}
                                ...
                                <div class="col-md-6 col-sm-6 col-xs-12">
                                    <div class="input-group">
                                        <input type="number" min="0" id="Giam" name="Giam" required
                                               value="{{ $khuyenmai->sale }}" class="form-control col-md-7 col-xs-12">
                                ...
                                <div class="col-md-6 col-sm-6 col-xs-12">
                                    <select class="form-control" id="MaSP" name="MaSP">
                                        @foreach($danhsachsanpham as $sanpham)
                                            <option value="{{$sanpham->id}}" {{ ($khuyenmai->product_id == $sanpham->id) ? "selected" : '' }}>{{$sanpham->product_name}} </option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
...
@push('js')
    ...
    <script>
        $('#ThoiGianBD').datetimepicker({
            format: 'YYYY-MM-DD',
            defaultDate: "{{ $khuyenmai->start_time }}"
        });
        $('#ThoiGianKT').datetimepicker({
            format: 'YYYY-MM-DD',
            defaultDate: "{{ $khuyenmai->end_time }}"
        });
    </script>
@endpush